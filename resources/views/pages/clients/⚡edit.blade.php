...
new #[Layout('layouts::app')] #[Title('Editar Cliente')] class extends Component {
    use WithFileUploads;
    ...
    public function save(): void
    {
        ...
        if ($this->dni_image) {
            if ($this->client->dni_image_path) {
                Storage::disk('public')->delete($this->client->dni_image_path);
            }
            $validated['dni_image_path'] = $this->dni_image->store('clients/dni', 'public');
        }

        if ($this->license_image) {
            if ($this->client->license_image_path) {
                Storage::disk('public')->delete($this->client->license_image_path);
            }
    ...
    public function deleteExistingDniImage(): void
    {
        if ($this->client->dni_image_path) {
            Storage::disk('public')->delete($this->client->dni_image_path);
            $this->client->update(['dni_image_path' => null]);
        }
    }

    public function deleteExistingLicenseImage(): void
    {
        if ($this->client->license_image_path) {
            Storage::disk('public')->delete($this->client->license_image_path);
            $this->client->update(['license_image_path' => null]);
        }
    }
}; ?>
...
<div class="space-y-6">
        <div class="flex items-center gap-4">
            <flux:button variant="ghost" icon="arrow-left" href="{{ route('clients.show', $client) }}" wire:navigate />
            <flux:heading size="xl">Editar Cliente: {{ $client->name }}</flux:heading>
        </div>
        ...
        <form wire:submit="save" class="space-y-6">
            <flux:card>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <flux:input wire:model="company_name" label="Nombre de empresa" />
                    <flux:input wire:model="email" type="email" label="Email" />
                    <flux:input wire:model="phone" label="Teléfono" />
                    <flux:input wire:model="address" label="Dirección" class="sm:col-span-2" />
                </div>
            </flux:card>
            ...
            <flux:card>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <flux:input wire:model="dni" label="DNI/NIE" />
                    <flux:input wire:model="license_number" label="Nº Carnet de conducir" />
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            ...
                            @elseif ($client->dni_image_path)
                                <div class="relative mb-2">
                                    <img src="{{ $client->dniImageUrl() }}" alt="DNI actual" class="h-32 w-auto rounded-lg border border-zinc-200 object-cover dark:border-zinc-700">
                                    <button type="button" wire:click="deleteExistingDniImage" wire:confirm="¿Eliminar la imagen del DNI?" class="absolute -right-2 -top-2 rounded-full bg-red-500 p-1 text-white shadow hover:bg-red-600">
                                        <x-flux::icon.x-mark class="size-3" />
                                    </button>
                                </div>
                            @endif
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            ...
                            @elseif ($client->license_image_path)
                                <div class="relative mb-2">
                                    <img src="{{ $client->licenseImageUrl() }}" alt="Carnet actual" class="h-32 w-auto rounded-lg border border-zinc-200 object-cover dark:border-zinc-700">
                                    <button type="button" wire:click="deleteExistingLicenseImage" wire:confirm="¿Eliminar la imagen del carnet?" class="absolute -right-2 -top-2 rounded-full bg-red-500 p-1 text-white shadow hover:bg-red-600">
                                        <x-flux::icon.x-mark class="size-3" />
                                    </button>
                                </div>
                            @endif
            ...
            <div class="flex justify-end gap-4">
                <flux:button variant="ghost" href="{{ route('clients.show', $client) }}" wire:navigate>Cancelar</flux:button>
